...
class SettingsRepository @Inject constructor(
    ...
    fun setGeminiModel(model: String) = prefs.edit().putString("gemini_model", model).apply()
    ...
    fun getSystemPromptId(): String =
        prefs.getString("system_prompt_id", "jarvis_ultimate") ?: "jarvis_ultimate"
    ...
    fun resolveSystemPrompt(): String {
        ...
        val base = if (id == "custom") {
            ...
            if (c.isNotBlank()) c else SystemPrompts.defaultBody()
        } else {
            SystemPrompts.byId(id).body
        }
        val depth = SystemPrompts.DepthMode.entries
            .find { it.id == getDepthMode() }
            ?: SystemPrompts.DepthMode.BALANCED
        return base + "\n\n" + SystemPrompts.depthAddon(depth)
    }
